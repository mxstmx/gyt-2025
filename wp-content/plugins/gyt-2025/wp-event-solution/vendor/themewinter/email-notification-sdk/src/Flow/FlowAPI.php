...
class FlowAPI extends WP_REST_Controller {
    ...
    public function create_item( $request ) {
        $response = ens_verify_nonce( $request->get_header( 'x_wp_nonce' ) );
    ...
    public function update_item( $request ) {
        ...
        if ( !$flow->is_flow() ) {
            ...
            $response = [
                'success'     => 0,
                'status_code' => 422,
                'message'     => __( 'Invalid flow id.', Config::get( 'text_domain' ) ),
                'data'        => [],
            ];
    ...
    public function get_item( $request ) {
        ...
        $response = [
            'success'     => 1,
            'status_code' => 200,
            'message'     => __( 'Successfully retrieved flow', Config::get( 'text_domain' ) ),
    ...
    public function get_items( $request ) {
        ...
        $data = [
            'success'     => 1,
            'status_code' => 200,
            'message'     => __( 'Showing filtered flow list.', Config::get( 'text_domain' ) ),
            'data'        => [
                'total'        => $flows['total'],
    ...
    public function delete_item( $request ) {
        ...
        $response = [
            'success'     => 1,
            'status_code' => 200,
            'message'     => __( 'Successfully deleted flow', Config::get( 'text_domain' ) ),
            'data'        => [],
        ];
    ...
    public function bulk_delete( $request ) {
        ...
        return rest_ensure_response( [
            'success'     => 1,
            'status_code' => 200,
            'message'     => __( 'Successfully deleted all flows', Config::get( 'text_domain' ) ),
            'data'        => [
                'items' => $flow_ids,
            ],
        ], 200 );
    ...
    public function clone_item( $request ) {
        ...
        $response = [
            'success'     => 1,
            'status_code' => 200,
            'message'     => __( 'Successfully cloned requested flow', Config::get( 'text_domain' ) ),
            'data'        => $item,
        ];
    ...
    public function save_flow( $request, $id = 0 ) {
        ...
        if ( empty( $data ) ) {
            return rest_ensure_response( [
                'status_code' => 400,
                'success'     => 0,
                'message'     => __( 'No data provided to save the flow', Config::get( 'text_domain' ) ),
            ], 400 );
        }
        ...
        $flow_config = !empty( $data['flow_config'] ) ? ens_sanitize_recursive( $data['flow_config'] ) : $flow->get_flow_config();
        ...
        if ( empty( $name ) || empty( $trigger ) ) {
            return rest_ensure_response( [
                'status_code' => 400,
                'success'     => 0,
                'message'     => __( 'Flow name & trigger is required', Config::get( 'text_domain' ) ),
            ], 400 );
        }
        ...
        $response = [
            ...
            'message'     => esc_html( sprintf( __( 'Successfully %s notification flow', Config::get( 'text_domain' ) ), $action ) ),
            'data'        => $item,
        ];

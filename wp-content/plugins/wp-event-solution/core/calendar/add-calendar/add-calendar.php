...
class Add_Calendar {
    ...
    function etn_add_to_google_calender_link( $pid ) {
        ...
        $outlook_event_start = ! empty( $event_meta['etn_start_date'][0] ) ?  date('Y-m-d', strtotime($event_meta['etn_start_date'][0])). 'T'. urlencode(date( "H:i:s", strtotime($event_meta['etn_start_time'][0]) )) : "";

        $outlook_event_end   = ! empty( $event_meta['etn_end_date'][0] ) ? 
        date('Y-m-d', strtotime($event_meta['etn_end_date'][0])). 'T'. urlencode(date("H:i:s", strtotime($event_meta['etn_end_time'][0]))) : date('Y-m-d', strtotime($event_meta['etn_start_date'][0])). 'T'. urlencode(date("H:i:s", strtotime($event_meta['etn_end_time'][0])));
        ...
        <ul id="etn_add_calender_links" class="etn-calender-list">
            ...
            <li>
                <a href="<?php echo \Wpeventin::plugin_url('core/calendar/iCalender/download-ics.php'); ?>?date_end=<?php echo esc_html( Helper::convert_to_calender_date( $event_end ) ); ?>&date_start=<?php echo esc_html( Helper::convert_to_calender_date( $event_start ) ); ?>&summary=<?php echo esc_html( Helper::convert_to_calendar_title( $event->post_title ) ); ?>&location=<?php echo esc_html( $location ); ?>&description=<?php echo esc_html( Helper::convert_to_calendar_title( $event->post_title ) ); ?>" rel="noopener noreferrer" target='_blank' class='etn-add-to-calender'>
                    <div class="calender-icon">
            ...
            <li>
                <a href ="https://outlook.live.com/calendar/0/deeplink/compose?rru=addevent&enddt=<?php echo $outlook_event_end; ?>&startdt=<?php echo $outlook_event_start; ?>&allday=false&subject=<?php echo esc_html( Helper::convert_to_calendar_title( $event->post_title ) ); ?>&body=<?php echo esc_html( substr( Helper::content_to_html( $event->post_content ), 0, 1000 ) ); ?>" rel="noopener noreferrer" target='_blank' class='etn-add-to-calender' rel="nofollow"> 
                    <div class="calender-icon">
